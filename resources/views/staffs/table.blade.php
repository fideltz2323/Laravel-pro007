...
<div class="box box-primary">
	...
	<div class="box-body">
	 ...
	 <?php echo Form::open(array('url'=>'staffs/delete/', 'class'=>'form-horizontal' ,'id' =>'MmbTable'  ,'data-parsley-validate'=>'' )) ;?>
			...
			<tr id="form-0" >
			  ...
			  @endif
                ...
                <tr class="editable" id="form-{{ $row->staffID }}">
					...
					<td ><input type="checkbox" class="ids" name="ids[]" value="<?php echo $row->staffID ;?>" />  </td>
					@if($setting['view-method']=='expand')
					<td><a href="javascript:void(0)" class="expandable" rel="#row-{{ $row->staffID }}" data-url="{{ url('staffs/show/'.$id) }}"><i class="fa fa-plus-square " ></i></a></td>
					@endif
				 <td data-values="action" data-key="<?php echo $row->staffID ;?>"  >
					{!! \App\Library\AjaxHelpers::buttonAction('staffs',$access,$id ,$setting) !!}
					{!! \App\Library\AjaxHelpers::buttonActionInline($row->staffID,'staffID') !!}
				</td>
							...
							$value = \App\Library\SiteHelpers::formatRows($row->{$field['field']}, $field , $row);
						 	...
						 	@if(\App\Library\SiteHelpers::filterColumn($limited ))
								 <td align="<?php echo $field['align'];?>" data-values="{{ $row->{$field['field']} }}" data-field="{{ $field['field'] }}" data-format="{{ htmlentities($value) }}">
									 @if($field['field'] == 'stafftypeID')
											@foreach($stafftypes as $a)
											<?php
												if($row->{$field['field']} == $a->stafftypeID){
													echo "<span>".$a->staff_type??''."</span>";
												}
											?>
											@endforeach
									 @elseif($field['field'] == 'cityID')
												 @foreach($cities as $a)
												 <?php
													 if($row->{$field['field']} == $a->cityID){
														 echo "<span>".$a->city_name??''."</span>";
													 }
												 ?>
												 @endforeach
									 @elseif($field['field'] == 'countryID')
												 @foreach($countries as $a)
												 <?php
													 if($row->{$field['field']} == $a->countryID){
                ...
                @if($setting['view-method']=='expand')
                <tr style="display:none" class="expanded" id="row-{{ $row->staffID }}">
                	<td class="number"></td>
                	<td></td>
                	<td></td>
