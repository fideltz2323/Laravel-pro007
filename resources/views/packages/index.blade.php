        ...
        <div class="hpanel">
                    <div class="panel-body">
                        <div class="stats-title pull-left">
                            <a href="{{ url('packages?search=status:equal:1|start:smaller_equal:'.$today.'|end:bigger:'.$today) }}"><h4>{{ Lang::get('core.runningtours') }}</h4></a>
                        </div>
    ...
    <table class="table table-striped table-bordered " id="{{ $pageModule }}Table">
        ...
        <tbody>
            @foreach ($rowData as $row)
                <tr>
					<td > {{ ++$i }} </td>
					<td ><input type="checkbox" class="ids minimal-red" name="ids[]" value="{{ $row->packageID }}" />  </td>
					<td style="width: 190px;">
                        @if($access['is_detail'] ==1)
							<a href="{{ url('packages/show/'.$row->packageID.'?return='.$return)}}" class="tips" title="{{ Lang::get('core.btn_view') }}"><i class="fa  fa-search fa-2x"></i> </a>
							@endif
							@if($access['is_edit'] ==1)
							<a   class="tips"
                            @if( $row->start < $today)
                            ...
                            @else
                            href="{{ url('packages/update/'.$row->packageID.'?return='.$return) }}" title="{{ Lang::get('core.btn_edit') }}"
                            @endif
                            ><i class="fa fa-edit fa-2x"></i> </a>
							@endif
                        {!! \App\Library\SiteHelpers::Featured($row->featured) !!}
                        {!! \App\Library\SiteHelpers::definite_departure($row->definite_departure) !!}
                        {!! \App\Library\GeneralStatuss::Tour_without_total($row->status,$row->start,$row->end,$row->packageID, $row->definite_departure) !!}
                    </td>
            ...
            <td>{{ $row->tour_code}}</td>
            <td>{{ $row->cost }}{{ \App\Library\SiteHelpers::formatLookUp($row->currencyID,'currencyID','1:def_currency:currencyID:symbol') }}</td>
