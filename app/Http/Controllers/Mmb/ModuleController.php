...
class ModuleController extends Controller {
    ...
    function postCreate( Request $request)
    {
        ...
        if ($validator->passes()) {
            ...
            $json_data['forms']    = $rowForm ;
						$json_data['setting']['form-method']    = 'modal';
            ...
            $data = array(
                ...
                'module_config' => \App\Library\SiteHelpers::CF_encode_json($json_data),
            );
    ...
    function getConfig( $id )
    {
		...
		$config = \App\Library\SiteHelpers::CF_decode_json($row->module_config,true);
    ...
    function postSaveconfig( Request $request)
    {
        ...
        if ($validator->passes()) {
            ...
            $lang = \App\Library\SiteHelpers::langOption();
            $language =array();
            foreach($lang as $l)
            {
                if($l['folder'] !='en'){
    ...
    public function postSavesetting( Request $request)
    {
        ...
        $row = $row[0];
        $config = \App\Library\SiteHelpers::CF_decode_json($row->module_config);
        $setting = array(
            'gridtype'        => '' ,
        ...
        $data['module_config'] = \App\Library\SiteHelpers::CF_encode_json($new_config);
        $data['module_type']    =  $request->input('module_type');


        \DB::table('tb_module')
            ->where('module_id', '=',$id )
            ->update(array('module_config' => \App\Library\SiteHelpers::CF_encode_json($new_config),'module_type'=> $request->input('module_type')));
    ...
    function postSavesql( Request $request ,$id )
    {
       ...
       \DB::table('tb_module')
           ->where('module_id', '=',$row->module_id )
            ->update(array('module_config' => \App\Library\SiteHelpers::CF_encode_json($config)));
    ...
    public function postSaveform( Request $request)
    {
        ...
        \DB::table('tb_module')
            ->where('module_id', '=',$id )
            ->update(array('module_config' => \App\Library\SiteHelpers::CF_encode_json($new_config)));
   ...
   function configGrid ( $field , $alias , $type, $sort ) {
    ...
    public function getRebuild(Request $request, $id = 0)
    {
            ...
            $codes['form_javascript'] = \App\Library\SiteHelpers::toJavascript($config['forms'],$path,$class);
            $codes['form_entry'] = \App\Library\SiteHelpers::toForm($config['forms'],$config['form_layout']);
            $codes['form_display'] = (isset($config['form_layout']['display']) ? $config['form_layout']['display'] : 'horizontal');
            $codes['form_view'] = \App\Library\SiteHelpers::toView($config['grid']);
            ...
            {
                $md = \App\Library\SiteHelpers::toMasterDetail($config['subform']);
            ...
            if($row->module_type =='generic')
            {
                ...
                $build_controller       = \App\Library\SiteHelpers::blend($controller,$codes);
                $build_view             = \App\Library\SiteHelpers::blend($view,$codes);
                $build_form             = \App\Library\SiteHelpers::blend($form,$codes);
                $build_grid             = \App\Library\SiteHelpers::blend($grid,$codes);
                $build_model            = \App\Library\SiteHelpers::blend($model,$codes);
            ...
            if($row->module_type =='addon')
            {
                ...
                $build_grid             = \App\Library\SiteHelpers::blend($grid,$codes);
                $build_model            = \App\Library\SiteHelpers::blend($model,$codes);
                $build_front            = \App\Library\SiteHelpers::blend($front,$codes);
                $build_frontview        = \App\Library\SiteHelpers::blend($frontview,$codes);
                $build_frontform        = \App\Library\SiteHelpers::blend($frontform,$codes);
            ...
            if($row->module_type =='ajax')
            {
                ...
                $build_form             = \App\Library\SiteHelpers::blend($form,$codes);
                $build_grid             = \App\Library\SiteHelpers::blend($grid,$codes);
                $build_table            = \App\Library\SiteHelpers::blend($table,$codes);
                $build_model            = \App\Library\SiteHelpers::blend($model,$codes);
                $build_toolbar          = \App\Library\SiteHelpers::blend($toolbar,$codes);
                $build_front            = \App\Library\SiteHelpers::blend($front,$codes);
                $build_frontview        = \App\Library\SiteHelpers::blend($frontview,$codes);
                $build_frontform        = \App\Library\SiteHelpers::blend($frontform,$codes);
    ...
    function postCode( Request $request , $id )
    {
        ...
        if(file_exists($filename))
        {
           $fp=fopen($filename,"w+");
            fwrite($fp,$content);
            fclose($fp);
            return response()->json(['status' => 'success' ,'message'=>  \App\Library\SiteHelpers::alert('success','File has been changed')]);
       // Return return json_encode(array());
        } else {
           return response()->json(['status' => 'error' ,'message'=>  \App\Library\SiteHelpers::alert('success','Error while saving changes')]);
        }
